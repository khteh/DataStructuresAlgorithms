...
bool isAdditiveNumber(string&);
bool checkIfAdditiveSequence(size_t, size_t, string&);
...
set<set<size_t>> numberCombinations1(size_t, size_t, size_t);
set<set<size_t>> numberCombinations(size_t, size_t);
...
void rotateMatrix(vector<vector<long>>&);
vector<long> matrixSprialOrder(vector<vector<long>>&);
...
void DutchPartitioning(vector<size_t>&, size_t);
...
void WordLadder(string &, string&, set<string>&, vector<string>&);
