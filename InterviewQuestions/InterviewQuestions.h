...
set<string> permute(const string &);
...
void findPrime(unsigned long, vector<long>&);
...
void TopDownMergeSort(vector<long>&, size_t, size_t);
void BottomUpMergeSort(vector<long>&);
void Merge(vector<long>&, size_t start, size_t middle, size_t end);
void Swap(long &, long &);
...
size_t diffpairs(set<long>&, long diff);
...
size_t ChocolatesByNumbers(size_t, size_t);
