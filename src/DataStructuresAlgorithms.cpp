...
4 5 6 6 6 5 4 -> 1 2 3 1 2 1 0 -> 1 2 3 1 3 2 1 = 13

4 5 6 6 6 4 5 -> 1 2 3 1 2 1 2 = 12

6 5 3 3 3 4 1 -> 3 2 1 2 1 2 1 = 12

6 5 4 4 4 2 3 -> 4 3 2 1 2 1 2 = 15
...
1 2 1 2 1 2 1 2
 */
size_t candies(vector<size_t> const &data)
{
	...
	for (size_t i = 1; i < data.size(); i++)
	{
		if (data[i] > data[i - 1])
		{
			if (direction != Direction::Up && result.back() > 1)
			{
				size_t diff = result.back() - 1;
				for (long j = i - 1; j > 0 && result[j] > 1 && result[j - 1] > result[j]; j--)
					result[i] -= diff;
			}
		...
		else
		{
			...
			result.push_back(result.back() > 1 ? 1 : result.back() + 1);
		}
	}
	if (result.back() > 1)
	{
		if (result.back() < result[result.size() - 2]) // Downward
		{
			size_t diff = result.back() - 1;
			for (long i = result.size() - 1; i > 0 && result[i] > 1 && result[i - 1] > result[i]; i--)
				result[i] -= diff;
		}
		else
		{ // Upward
			long i = result.size() - 1;
			for (; i > 0 && result[i] > result[i - 1]; i--)
				;
			size_t diff = result[i] - 1;
			for (; i < result.size(); result[i++] -= diff)
				;
		}
	}
